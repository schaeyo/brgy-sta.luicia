...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $gender = $_POST['gender'];
    $email = $_POST['email'];
    $residence_since = $_POST['residence_since'] ?? null;
    ...
    if (move_uploaded_file($valid_id['tmp_name'], $valid_id_path)) {
        ...
        if ($stmt->execute()) {
            ...
            header("Location: submitReqForm.php?status=success");
            exit();
        } else {
            // Redirect back to the form with an error message
            header("Location: submitReqForm.php?status=error");
            exit();
        }
        ...
        header("Location: submitReqForm.php?status=file_error");
        exit();
    }
